...
def test_recover_batch_failure_records_failed_payload_and_chains_exception(tmp_path: Path) -> None:
    state_path = tmp_path / "state.db"
    target = tmp_path / "config.txt"
    target.write_text("old\n", encoding="utf-8")
    now = datetime(2026, 1, 1, tzinfo=UTC)
    ...
    lease = _acquire_and_claim(state_path, resource_key, now=now)
    journal = OperationJournalStore(state_path)
    coordinator = _coordinator(state_path, journal=journal, clock=_sequence_clock([completion_now]))

    batch = journal.create_batch(
        idempotency_key="recover-failure:config",
        lease=lease,
        owner="owner-a",
        run_id="run-1",
        resource_key=resource_key,
    ...
    def fail_recover(context: JournaledFilesystemRecoveryContext) -> None:
        raise RuntimeError("injected recovery failure")

    with pytest.raises(JournaledFilesystemRecoveryError, match="recovery callback failed") as raised:
        coordinator.recover_batch(batch.batch_id, lease=lease, recover=fail_recover, now=now)

    assert isinstance(raised.value.__cause__, RuntimeError)
    assert str(raised.value.__cause__) == "injected recovery failure"
    assert raised.value.recovery_record is not None
    assert raised.value.recovery_record.phase == BatchPhase.RECOVERY_FAILED
    assert raised.value.recovery_record.created_at == now
    updated = journal.get_batch(batch.batch_id)
    assert updated is not None
    assert updated.phase == BatchPhase.RECOVERY_FAILED
    assert updated.status_payload["error"] == "injected recovery failure"
    assert len(updated.status_payload["operations"]) == 1
    assert updated.status_payload["operations"][0]["operation_type"] == "write_text"
    assert [record.phase for record in journal.list_recovery_records(batch.batch_id)] == [
        BatchPhase.RECOVERY_DESIRED,
        BatchPhase.RECOVERING,
        BatchPhase.RECOVERY_FAILED,
...
def test_recover_batch_preserves_callback_cause_when_failure_recording_raises(tmp_path: Path) -> None:
    state_path = tmp_path / "state.db"
    target = tmp_path / "config.txt"
    target.write_text("old\n", encoding="utf-8")
    now = datetime(2026, 1, 1, tzinfo=UTC)
    ...
    class FailureRecordingJournal(OperationJournalStore):
        def record_recovery_failed(self, batch_id: str, **kwargs):
            raise RuntimeError("injected recovery failure recording error")

    journal = FailureRecordingJournal(state_path)
    coordinator = _coordinator(state_path, journal=journal, clock=_sequence_clock([completion_now]))

    batch = journal.create_batch(
        idempotency_key="recover-failure-recording:config",
        lease=lease,
        owner="owner-a",
        run_id="run-1",
        resource_key=resource_key,
    ...
    def fail_recover(context: JournaledFilesystemRecoveryContext) -> None:
        raise ValueError("injected callback error")

    with pytest.raises(JournaledFilesystemRecoveryError, match="recovery callback failed") as raised:
        coordinator.recover_batch(batch.batch_id, lease=lease, recover=fail_recover, now=now)

    assert isinstance(raised.value.__cause__, ValueError)
    assert str(raised.value.__cause__) == "injected callback error"
    assert isinstance(raised.value.recording_error, RuntimeError)
    assert str(raised.value.recording_error) == "injected recovery failure recording error"
    updated = journal.get_batch(batch.batch_id)
    assert updated is not None
